...
# A game too large for one zone. A ward holds 1384 Sparks, so 1500 is two wards, and the run
# moves one Spark between them halfway through. The sum has to hold across both wards, which
# is the part a migration is able to break. A Spark is a database, so this founds fifteen
# hundred of them: it is the step that says what a zone-sized ward costs to open.
step "A game of two wards, and a Spark that moves between them"
set +e
out=$(/build/queen shard 40 31337 1500 2>&1); rc=$?
set -e
echo "$out"
...
# One ward may not exceed its zone by itself. The refusal is the feature.
step "One ward refuses to exceed a zone"
if /build/queen play 5 1 1500 2>/dev/null; then
  echo "a 1500-Spark ward was allowed, and a zone holds 1384"; exit 1
fi
