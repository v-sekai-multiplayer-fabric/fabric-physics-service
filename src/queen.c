...
static int found_ward(gyre_t *g, const char *prefix, int nsparks, uint64_t seed) {
	...
	if (run(g->ward,
	        ...
	        "                   risk INT, taken INT, outcome TEXT);"
	...
	for (int i = 0; i < NVENUES; i++) {
		char safe[96];
		snprintf(sql, sizeof sql, "INSERT INTO venue VALUES (%d, '%s', %d, 0)", i,
		         quoted(VENUES[i].name, safe, sizeof safe), VENUES[i].cost);
	...
	for (int i = 0; i < g->nsparks; i++) {
		...
		if (run(s->db, "CREATE TABLE spark(id INT PRIMARY KEY, purse INT, wear INT, cycles INT);"
			...
			return 1;
		snprintf(sql, sizeof sql, "INSERT INTO spark VALUES (%d, 0, 0, 0)", i);
...
static int post_board(gyre_t *g) {
	...
	for (int i = 0; i < n; i++) {
		...
		snprintf(sql, sizeof sql,
		         "INSERT INTO board(cycle, kind, payout, risk, taken, outcome)"
		         " VALUES (%d, '%s', %d, %d, -1, NULL)",
		         g->cycle, kind, payout, risk);
		if (run(g->ward, sql)) return 1;
	}
	return 0;
}
...
static int cycle(gyre_t *g) {
	...
	for (int i = 0; i < g->nsparks; i++) {
		...
		snprintf(sql, sizeof sql, "UPDATE spark SET wear = %d, cycles = %d WHERE id = %d",
		         s->wear, g->cycle, s->id);
		if (run(s->db, sql)) return 1;
	}
