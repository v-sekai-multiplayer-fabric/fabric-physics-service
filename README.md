...
with the authority moved to the server. It is a clone of `fabric-store-domain` and everything
below still describes that repository, because the physics is not written yet and pretending
otherwise would leave a reader with a design nothing here implements.
...
a ward is a SQLite database over the store plane's VFS, a cycle is a transaction, and no client
...
## What follows is `fabric-store-domain`
